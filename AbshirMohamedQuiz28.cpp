...
int main()
{
    ...
    cout<<"The minimum amount of sales is " << findLowest(sales,QTRS)<<endl;
    
    cout<<"The maximum amount of sales is " << findHighest(sales,QTRS)<<endl;
    return 0;
}
...
double findHighest(double region[],int size)
{
    double *ref = region;
    ...
    for(int i = 0; i<size;i++)
        maximum = max(maximum,*(region+i));
    
    return maximum;
}

double findLowest(double region[],int size)
{
    double *ref = region;
